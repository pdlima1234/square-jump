...
void render_in_play(SDL_Renderer* renderer, SDL_Rect* bg_rect, SDL_Rect* player_rect, SDL_Rect* floor_rect, SDL_Colour bg_colour, SDL_Colour player_colour,
	SDL_Colour floor_colour, SDL_Rect** obstacles);
void jump(SDL_Renderer* renderer, SDL_Rect* bg_rect, SDL_Rect* player_rect, SDL_Rect* floor_rect, SDL_Colour bg_colour, SDL_Colour player_colour,
	SDL_Colour floor_colour, SDL_Rect** obstacles, SDL_Event event);
...
void colliding_obstacle(SDL_Rect** obstacles, SDL_Renderer* renderer, SDL_Rect* bg_rect, SDL_Rect* player_rect, SDL_Rect* floor_rect,
	SDL_Colour bg_colour, SDL_Colour player_colour, SDL_Colour floor_colour, SDL_Event event);
...
main(int argc, char* argv[]) {
	...
	while (is_alive) {
		render_in_play(renderer, bg_rect, player_rect, floor_rect, bg_colour, player_colour, floor_colour, obstacles);
		// Logic for player movement according to key that was pressed
		while (SDL_PollEvent(&event) != 0) {
			if (event.type == SDL_KEYDOWN) {
				if (event.key.keysym.sym == SDLK_SPACE) {
					jump(renderer, bg_rect, player_rect, floor_rect, bg_colour, player_colour, floor_colour, obstacles, event);
				}
					...
					fall_width_x = fall(renderer, bg_rect, player_rect, floor_rect, bg_colour, player_colour, floor_colour, obstacles, event);
...
loss(SDL_Renderer* renderer, SDL_Colour bg_colour, SDL_Colour font_colour, TTF_Font* title_font) {
	...
	SDL_Delay(GAME_OVER_DISPLAY_TIME);
}
...
colliding_obstacle(SDL_Rect** obstacles, SDL_Renderer* renderer, SDL_Rect* bg_rect, SDL_Rect* player_rect, SDL_Rect* floor_rect,
	...
	int i, is_colliding_obstacle = FALSE, x=0, y, player_start_y = player_rect->y, has_jump = FALSE;
...
SDL_Rect**
get_obstacles(void) {
	int i, is_hole, prev_obstacle_x = (W_WIDTH / 2) + OBSTACLE_SPACING, obstacle_x, obstacle_width, obstacle_height;
